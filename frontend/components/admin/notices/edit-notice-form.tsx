"use client";

import { Alert, Button, Card } from "@heroui/react";
import { useState, useTransition } from "react";

import { updateNoticeAction } from "@/actions/notices.action";
import { NoticeSeveritySelect } from "@/components/notices/notice-severity-select";
import {
  NoticeTranslationsEditor,
  type TranslationDraft,
  toTranslationInputs,
} from "@/components/notices/notice-translations-editor";
import { useI18n } from "@/i18n/provider";
import type {
  Notice,
  NoticeSeverity,
  PageScope,
  UpdateNoticePayload,
} from "@/lib/notices.api";

const SCOPES: PageScope[] = [
  "all",
  "home",
  "centers",
  "requests",
  "parts",
  "my_contributions",
  "about",
];

/** Edit an existing notice (page or entity) from the admin Notices tab. */
export function EditNoticeForm({
  notice,
  onClose,
}: {
  notice: Notice;
  onClose: () => void;
}) {
  const { dict } = useI18n();
  const t = dict.notices;
  // Entity notices (target_type set) cannot change their scope; only page
  // banners expose the page checkboxes.
  const isPageMode = notice.target_type === null;
  const scopeLabel: Record<PageScope, string> = {
    all: t.scopeAll,
    home: t.scopeHome,
    centers: t.scopeCenters,
    requests: t.scopeRequests,
    parts: t.scopeParts,
    my_contributions: t.scopeMyContributions,
    about: t.scopeAbout,
  };

  const [severity, setSeverity] = useState<NoticeSeverity>(notice.severity);
  const [scopes, setScopes] = useState<PageScope[]>(
    notice.scopes as PageScope[],
  );
  const [drafts, setDrafts] = useState<TranslationDraft[]>(
    notice.translations.map((tr) => ({
      language: tr.language,
      title: tr.title ?? "",
      message: tr.message,
      action_label: tr.action_label ?? "",
      action_url: tr.action_url ?? "",
    })),
  );
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  function toggleScope(scope: PageScope) {
    setScopes((prev) =>
      prev.includes(scope) ? prev.filter((s) => s !== scope) : [...prev, scope],
    );
  }

  function submit() {
    const translations = toTranslationInputs(drafts);
    if (isPageMode && scopes.length === 0) {
      setError(t.errorScopesRequired);
      return;
    }
    if (translations.some((tr) => !tr.message || !tr.language)) {
      setError(t.errorMessageRequired);
      return;
    }
    setError(null);
    const payload: UpdateNoticePayload = { severity, translations };
    if (isPageMode) {
      payload.scopes = scopes;
    }
    startTransition(async () => {
      const res = await updateNoticeAction(notice.id, payload);
      if (res.error) {
        setError(res.error);
        return;
      }
      onClose();
    });
  }

  return (
    <Card>
      <Card.Header>
        <Card.Title>{t.editTitle}</Card.Title>
      </Card.Header>
      <Card.Content>
        <div className="flex flex-col gap-4">
          <NoticeSeveritySelect value={severity} onChange={setSeverity} />

          {isPageMode && (
            <div className="flex flex-col gap-2 text-sm">
              <span className="font-medium">{t.scopesLabel}</span>
              <div className="flex flex-wrap gap-x-4 gap-y-2">
                {SCOPES.map((scope) => (
                  <label key={scope} className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={scopes.includes(scope)}
                      onChange={() => toggleScope(scope)}
                    />
                    {scopeLabel[scope]}
                  </label>
                ))}
              </div>
            </div>
          )}

          <NoticeTranslationsEditor value={drafts} onChange={setDrafts} />

          {error && (
            <Alert status="danger">
              <Alert.Indicator />
              <Alert.Content>
                <Alert.Description>{error}</Alert.Description>
              </Alert.Content>
            </Alert>
          )}

          <div className="flex gap-2">
            <Button isPending={isPending} onPress={submit}>
              {t.save}
            </Button>
            <Button variant="ghost" onPress={onClose}>
              {t.cancel}
            </Button>
          </div>
        </div>
      </Card.Content>
    </Card>
  );
}
