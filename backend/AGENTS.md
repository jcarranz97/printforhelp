...
logic in `contributions/service.py` so it is unit-testable; wire it to a
periodic trigger at deploy). Add new domains under `app/<domain>/`
...
for those four `service.py` files in `pyproject.toml`.
